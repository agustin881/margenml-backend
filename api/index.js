...
app.get('/api/version', (req, res) => res.json({ version: 'v35-familias', costo_congelado: true }));
...
- Si el usuario da un SKU madre sin sufijo de color (ej "STL150", "sacale el descuento al STL150"), pasalo TAL CUAL en "sku": el sistema lo expande solo a toda la familia de colores.
...
async function asistenteResolverFamilia(p, userId, token) {
  ...
  try {
    if (_contaMapaCache.mapa) {
      Object.keys(_contaMapaCache.mapa).forEach(s => { if (s !== base && s.indexOf(base) === 0) hijos[s] = 1; });
    }
  } catch (e) {}
...
app.listen(PORT, () => console.log(`MargenML backend v35 (familias) corriendo en puerto ${PORT}`));
